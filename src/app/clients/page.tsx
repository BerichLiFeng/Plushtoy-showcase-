...
interface Client { id: number; name: string; description: string; logo_key: string; }
...
export default function ClientsPage() {
  ...
  useEffect(() => {
    fetch("/api/public/clients").then(r=>r.json()).then(d=>{setItems(d);setLoading(false)}).catch(()=>setLoading(false));
  }, []);
    ...
    <div className="min-h-screen bg-background">
      <section className="relative pt-28 pb-16 md:pt-36 md:pb-20 bg-gradient-to-b from-primary/5 to-background">
        <div className="max-w-7xl mx-auto px-4 md:px-8 text-center">
          ...
          <h1 className="font-serif text-4xl md:text-5xl mt-4 text-foreground font-light">{t(lang, "nav.clients")}</h1>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 grid md:grid-cols-3 gap-8">
          {items.map(item => (
            <div key={item.id} className="bg-white rounded-2xl p-8 shadow-sm border border-primary/5 text-center">
              ...
              <h3 className="font-serif text-lg mb-2 text-foreground">{item.name}</h3>
